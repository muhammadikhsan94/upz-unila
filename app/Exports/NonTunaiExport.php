...
class NonTunaiExport implements FromCollection, WithHeadings
{
    /**
    ...
    public function collection()
    {
        if($this->id == NULL) {
            $data = DB::table('transaksi')
                ...
                ->select('transaksi.no_kuitansi','users.nama as duta','users.no_punggung','wilayah.nama_wilayah','donatur.nama as donatur','lembaga.nama_lembaga','paketzakat.nama_paket_zakat','transaksi.item','transaksi.nama_bank','transaksi.jumlah')
                ->where('transaksi.item', 'Non Tunai')
