...
class ManajerController extends Controller
{
    ...
    public function kurban()
    {
        ...
        return view('admin.manajer.kurban', compact('data'));
    }
    ...
    public function kurbanIndex()
    {
        ...
        $manajer            = Role::where('id_jabatan', 5)->where('id_atasan', Auth::user()->id)->pluck('id_users');
        $duta               = User::whereNull('deleted_at')->where('id_wilayah', Auth::user()->id_wilayah)->whereIn('id', $manajer)->get();

        $tmp = [];
        foreach ($duta as $item) {
            $y			    = DB::table('transaksi_kurban as transaksi')
                            ->where('transaksi.id_users', $item->id)
                            ->join('detail_transaksi_kurban as detail','detail.id_transaksi','=','transaksi.id')
                            ->join('status_transaksi_kurban as status','status.id_transaksi','=','transaksi.id')
        ...
        //Realisasi
        $manajer_   = User::where('id', Auth::user()->id)->get();
        $tmp_ = [];
        foreach ($manajer_ as $item_) {
			$jabatan		= Role::where('id_jabatan', 5)->where('id_atasan', $item_->id)->pluck('id_users');
        ...
        return view('admin.manajer.beranda_kurban', compact('data'));
    }
